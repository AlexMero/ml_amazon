...
df = df[["reviews", "stars", "isBestSeller",
         "title", "price", "category", "boughtInLastMonth"]].copy()

le = LabelEncoder()
# Encode the category and isBestSeller columns
df['category'] = le.fit_transform(df['category'])
df['isBestSeller'] = le.fit_transform(df['isBestSeller'])

# Remplacement de Normalizer par StandardScaler pour les colonnes numériques
# norm = Normalizer()  # instantiate the normalizer


stdScaler = StandardScaler()  # instantiate the standard scaler
# # normalize the numerical features
# df[numericals] = norm.fit_transform(df[numericals])
df['stars'] = stdScaler.fit_transform(df[['stars']])
df['boughtInLastMonth'] = stdScaler.fit_transform(df[['boughtInLastMonth']])
df['price'] = stdScaler.fit_transform(df[['price']])
df['reviews'] = stdScaler.fit_transform(df[['reviews']])

# Après vérification il n'y a qu'une valeur manquante
df.dropna(inplace=True)  # drop the missing values

# Tokenize title
tokenizer = get_tokenizer("basic_english")

# define a function to extract the tokens


def extract_tokens(x):
    for txt, _, _, _, _ in x:  # iterate over the title column
        yield tokenizer(txt)
...
training_phase, testing = train_test_split(
    df.values, random_state=42, test_size=0.2)
...
# fr : batch est le nombre d'échantillons qui seront propagés dans le réseau
BATCH = 128
...
# fr : le taux d'apprentissage est un hyperparamètre qui contrôle combien nous ajustons les poids de notre réseau par rapport au gradient de perte
LR = 0.1
...
# fr : un epoch est une mesure du nombre de fois où tous les vecteurs d'entraînement sont utilisés une fois pour mettre à jour les poids
EPOCHS = 5
...
numericals = ["stars", "boughtInLastMonth", "price",
              "reviews"]  # get the numerical features
